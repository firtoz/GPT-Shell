...
Total messages sent by all users of this server's API key: ${totalSum}.`,
            ...
            fields.push({
                name: 'Sent Messages',
                value: `You sent ${(0, GetMessageCountForUser_1.getMessageCountForUser)(messageCounter, user.id).limitCount}/${config.maxMessagePerUser === -1 ? 'Unlimited' : config.maxMessagePerUser} messages.
...
As new messages are sent, they will be stored in long term memory one by one, so this value is useful only for conversations before V2. If you don't care about old conversations, set this to be 0.`,
                ...
                components: [
                    new discord_js_1.ActionRowBuilder()
                        .addComponents(PineconeModal_1.PineconeModal.getButtonComponent(), EmbedLimitModal_1.EmbedLimitModal.getButtonComponent(), PromptPermissionsModal_1.PromptPermissionsModal.getButtonComponent()),
                ]
            });
        }
...
If you'd like to use the server's API key, please send me the /${CONFIG_COMMAND_NAME} command in a DM.`,
                    ...
                    if (configId != null) {
                        ...
                        const fields = [{
                                name: 'Sent Messages',
                                value: `You sent ${(0, GetMessageCountForUser_1.getMessageCountForUser)(messageCounter, commandInteraction.user.id).limitCount}/${config.maxMessagePerUser === -1 ? 'Unlimited' : config.maxMessagePerUser} messages.`
                            }];
                        await commandInteraction.followUp({
                            ephemeral: true,
                            embeds: [
