...
const ImageRequest_1 = require("./ImageRequest");
...
${failures.map(([key]) => {
        ...
        if (descriptions.length && messageToReplyTo) {
            ImageRequest_1.ImageRequest.handle(openai, descriptions, userId, messageToReplyTo).catch();
        }
        const sendPromise = this.SendPromptToGPTChat(currentConfig, usingOpenAIForServer, openai, user, inputValue, async (result, finished) => {
            ...
            if (finished) {
                ...
                if (multi.messageList.length > 0) {
                    ...
                    if (descriptions.length && messageToReplyTo) {
                        ImageRequest_1.ImageRequest.handle(openai, descriptions, userId, lastMessage.message).catch();
                    }
                    await this.persist();
                }
            }
...
Thank you for your understanding.`),
    ...
    async getImageDescriptions(inputValue, usingOpenAIForServer, userId, currentConfig, channel) {
        let descriptions = (0, ImageRequest_1.extractDescriptions)(inputValue);
